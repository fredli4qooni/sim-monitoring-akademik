    ...
    <div class="py-8">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-saas rounded-3xl border border-slate-100/50 p-8">
                ...
                <form method="POST" action="{{ route('data-latih.store') }}" class="space-y-6">
                    @csrf
                    ...
                    <div>
                        ...
                        <x-text-input id="angkatan" class="block mt-1 w-full" type="number" name="angkatan" :value="old('angkatan')" min="2000" max="{{ date('Y') }}" required />
                        <x-input-error :messages="$errors->get('angkatan')" class="mt-2" />
                    </div>

                    <div>
                        <x-input-label for="semester_lulus" :value="__('Semester Lulus')" />
                        <x-text-input id="semester_lulus" class="block mt-1 w-full" type="number" name="semester_lulus" :value="old('semester_lulus')" min="1" max="14" required />
                    ...
                    <div class="pt-6 border-t border-slate-100">
                        ...
                        <div class="space-y-6">
                            <div>
                                ...
                                <x-text-input id="ip_terakhir" name="ip_terakhir" type="number" step="0.01" min="0" max="4" class="mt-1 block w-full rounded-xl" :value="old('ip_terakhir')" required />
                            ...
                            <div>
                                ...
                                <select id="kondisi_ekonomi" name="kondisi_ekonomi" class="mt-1 block w-full border-gray-300 focus:border-primary-500 focus:ring-primary-500 rounded-xl shadow-sm" required>
                                    ...
                                    @for($i=1; $i<=5; $i++)
                                        <option value="{{ $i }}" {{ old('kondisi_ekonomi') == $i ? 'selected' : '' }}>Skala {{ $i }}</option>
                                    @endfor
                                </select>
                            ...
                            <div>
                                ...
                                <select id="lingkungan_sosial" name="lingkungan_sosial" class="mt-1 block w-full border-gray-300 focus:border-primary-500 focus:ring-primary-500 rounded-xl shadow-sm" required>
                                    <option value="">-- Pilih Skala --</option>
                                    @for($i=1; $i<=5; $i++)
                                        <option value="{{ $i }}" {{ old('lingkungan_sosial') == $i ? 'selected' : '' }}>Skala {{ $i }}</option>
                                    @endfor
                                </select>
                            ...
                            <div>
                                ...
                                <select id="keaktifan_organisasi" name="keaktifan_organisasi" class="mt-1 block w-full border-gray-300 focus:border-primary-500 focus:ring-primary-500 rounded-xl shadow-sm" required>
                                    ...
                                    <option value="1" {{ old('keaktifan_organisasi') == '1' ? 'selected' : '' }}>Aktif Berorganisasi</option>
                                    <option value="0" {{ old('keaktifan_organisasi') == '0' ? 'selected' : '' }}>Tidak Aktif</option>
                                </select>
                            ...
                            <div>
                                ...
                                <select id="layanan_akademik" name="layanan_akademik" class="mt-1 block w-full border-gray-300 focus:border-primary-500 focus:ring-primary-500 rounded-xl shadow-sm" required>
                                    <option value="">-- Pilih Skala --</option>
                                    @for($i=1; $i<=5; $i++)
                                        <option value="{{ $i }}" {{ old('layanan_akademik') == $i ? 'selected' : '' }}>Skala {{ $i }}</option>
                                    @endfor
                                </select>
                            ...
                            <div>
                                ...
                                <select id="asal_sekolah" name="asal_sekolah" class="mt-1 block w-full border-gray-300 focus:border-primary-500 focus:ring-primary-500 rounded-xl shadow-sm" required>
                                    ...
                                    <option value="SMA" {{ old('asal_sekolah') == 'SMA' ? 'selected' : '' }}>SMA</option>
                                    <option value="SMK" {{ old('asal_sekolah') == 'SMK' ? 'selected' : '' }}>SMK</option>
                                    <option value="MA" {{ old('asal_sekolah') == 'MA' ? 'selected' : '' }}>MA</option>
                                    <option value="Lainnya" {{ old('asal_sekolah') == 'Lainnya' ? 'selected' : '' }}>Lainnya</option>
                                </select>
                    ...
                    <div class="flex items-center justify-end pt-4 border-t border-slate-100">
                        <a href="{{ route('data-latih.index') }}" class="inline-flex items-center px-4 py-2 bg-white border border-slate-300 rounded-xl font-semibold text-xs text-slate-700 uppercase tracking-widest shadow-sm hover:bg-slate-50 mr-3">
                            Batal
                        </a>
                        <x-primary-button>
                            {{ __('Simpan Data') }}
